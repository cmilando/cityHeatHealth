...
The package `cityHeatHealth` makes it simple to estimate heat-health impacts at small spatial scales. This package can be used in three main ways:

| Single-Zone | 2-stage design | Spatial Bayes |
|-------------|----------------|---------------|
| A standard single-stage conditional poisson model when estimating heat-health impacts in a single zone | A 2-stage design (using mixmeta and BLUP) is used when estimating heat-health impacts across many zones, but where individual zone impacts are desired         | If numbers are very small in the 2-stage design, spatial bayesian methods can be used to tighten confidence intervals |

Followin any of these implementations, the same attributable number calculation is applied to model outcomes to get heat-attributable outcomes. To avoid repetition, we provide a separate vignette that details the attributable number module: `vignette("attributable_number")`.

This package comes pre-loaded with sample datasets so each of these methods can be explored. Starting from a messy exposure and outcome dataset, we can quickly estimate heat-health impacts. See below for starter code for each type of analysis.
...
```{r multi_town_run}
ma_model <- condPois_2stage(ma_exposure_matrix, ma_outcomes_table)
```
