...
You can also plot by group (TODO -- a way to make this cleaner to get to)

```{r grp, fig.height=8, fig.width=8}
ma_model$`_`$grp_plt
```

The centering points here are all different because they are the average of the centering points that are in each group. Should it work this way?

#### Model by factor

Only a small change is required to run the model by factor
