---
title: "One set of Heat-Health Associations for a single or multiple zones using `cityHeatHealth`"
output:
  rmarkdown::html_vignette:
    fig.retina: 2
    dev: svg
vignette: >
  %\VignetteIndexEntry{One set of Heat-Health Associations for a single or multiple zones using `cityHeatHealth`}
...
Let's use built-in functions to examine a single set of beta coefficients for heat-health relationships in a single or multiple geographic unit. We'll start with a review of DLNM, and then show how its implemented in `cityHeatHealth` for both the single and multi-zone cases.
...
plot(m1)
```
### Multi-zone DLNM (using `cityHeatHealth` functions in `R`)
...
```{r single_plot2, fig.height=3, fig.width=5}
plot(m2)
```
