...
parameters {
  ...
  /*
  vector[K] beta_bar;        // global mean effects
  matrix[K, J] beta_raw;     // standardized deviations
  vector<lower=0>[K] tau;    // SD of regional variation
  */
...
transformed parameters {
  ...
  matrix[K, J] beta;
  for (j in 1:J) {
    beta[, j] = beta_bar + tau .* beta_raw[, j];
  }
  */
